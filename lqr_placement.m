...
% Laura added: 
if parameterflag == 1 
    bcltestindex = 37;  %choose bcl=240 for voltage-driven alt
elseif parameterflag == 4
    bcltestindex = 40;  %choose bcl=210 for calcium-driven alt
end
% Reload correct Jacobian: 
eval(['load ' jacfolder 'jac' num2str(selected_bcls(bcltestindex)) '_pflag' num2str(parameterflag) ]) %Load data from jacobians    bcl = selected_bcls(i);
bcl = selected_bcls(bcltestindex);
% print current BCL to screen
disp(['BCL = ' num2str(bcl) ' ms'])

% Laura: In past versions, this just used the final Jacobian value (and
% bcl) that happened to show up in the selected_bcls for loop (or m=37... loop) above. 
% Fixed to use the desired bcl and Jacobian. 
%[xlqr, lqr_bcl] = lqrsf(x0,jac,Bmatrix,Klqr{m},nsteps,bcl);
xlqr = lqrsf(x0,jac,Bmatrix,Klqr{bcltestindex},nsteps);
...
% Laura added: bcl is never used in this function, so it shouldn't be an
% input argument. 
%function [xlqr,bcl] = lqrsf(x0,jac,Bmatrix,Klqr,nsteps,bcl)
function [xlqr] = lqrsf(x0,jac,Bmatrix,Klqr,nsteps)
